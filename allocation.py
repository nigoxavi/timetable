...
def _teaches_same_paper(timetable: dict, staff: str, subject: str, class_year: str) -> bool:
    """Return whether a professor teaches the same paper to the same class."""
    return any(
        lesson
        and _normalized(lesson.get("subject")) == _normalized(subject)
        and _normalized_co_taught_year(lesson.get("year"))
        == _normalized_co_taught_year(class_year)
        for day in timetable.get(staff, {}).values()
        for lesson in day.values()
    )


def _is_service_subject(subject: str) -> bool:
    """Identify papers handled for other departments from timetable markers."""
    normalized = _normalized(subject)
    return bool(re.search(r"(?:^|\s)(?:ao|ar|cd)(?:$|\s)", normalized))


def _teaches_major_in_class(timetable: dict, staff: str, class_year: str) -> bool:
    """Return whether staff teaches a non-service major paper to this class."""
    return any(
        lesson
        and _normalized_co_taught_year(lesson.get("year"))
        == _normalized_co_taught_year(class_year)
        and not _is_service_subject(lesson.get("subject", ""))
        for day in timetable.get(staff, {}).values()
        for lesson in day.values()
    )
    ...
    for missing in sorted(absent):
        for period, lesson in sorted(timetable.get(missing, {}).get(day_order, {}).items()):
            ...
            for candidate in timetable:
                ...
                same_paper = _teaches_same_paper(
                    timetable, candidate, lesson.get("subject", ""), lesson.get("year", "")
                )
                major_same_class = (
                    not _is_service_subject(lesson.get("subject", ""))
                    and _teaches_major_in_class(timetable, candidate, lesson.get("year", ""))
                )
                free_candidates.append(
                    (candidate, same_paper, major_same_class, same_class, daily_load, weekly, monthly)
                )

            standard_candidates = [item for item in free_candidates if item[4] < max_daily_periods]
            ...
            if not candidates:
                ...
                candidates = [
                    ...
                    and item[4] >= max_daily_periods
                    and item[4] < max_daily_periods + 1
                ]
            ...
            chosen, same_paper, major_same_class, same_class, daily_load, weekly, monthly = min(
                candidates,
                key=lambda item: (
                    not item[1],  # Exact same paper and class first (for example Python (AO)).
                    not item[2],  # For core papers, major-paper faculty of the same class.
                    not item[3],  # Then preserve the existing same-class priority.
                    item[4], item[5], item[6], item[0].casefold(),
                ),
            )
            ...
            rows.append({
                ...
                "reason": (
                    ...
                    + (
                        "same-paper and same-class professor; " if same_paper
                        else "same-class major-paper professor; " if major_same_class
